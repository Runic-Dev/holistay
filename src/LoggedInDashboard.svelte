<script lang="ts">
    ...
    function addNewProperty(payload: { detail: ConfirmedPropertyToSend }) {
        invoke("add_new_property", {request: payload.detail}).then(({propertyId, imageOption}: {
            propertyId: string,
            imageOption: string | null
        }) => {
            propertyStore.set({
                properties: [...properties, new Property(propertyId, payload.detail.name, "", imageOption)]
            })
